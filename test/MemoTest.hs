...
testSimples = TestCase $ do
    output <- processFile (executeProg ["conditional"] []) "src/Language/Analysis/Simple.lng" inputInt
...
testFibonacci = TestCase $ do
    output <- processFile (executeProg ["fib"] []) "src/Language/Analysis/Fibonacci.lng" inputInt
    let expectedOutput = (ValorInt 55) 
    assertEqual "Fibonacci x" expectedOutput (fst output)

testFatorial :: Test
testFatorial = TestCase $ do
    output <- processFile (executeProg ["fat"] []) "src/Language/Analysis/Fatorial.lng" inputInt
...
testSomaListaSimples = TestCase $ do
    output <- processFile (executeProg ["soma"] []) "src/Language/Analysis/Lista.lng" inputInt
    let expectedOutput = (ValorInt 46) 
    assertEqual "Soma lista [x, x+1, x+2, x+3]" expectedOutput (fst output)

testSomaLista :: Test
testSomaLista = TestCase $ do
    output <- processFile (executeProg ["soma"] []) "src/Language/Analysis/Lista-inputList.lng" inputList
    let expectedOutput = (ValorInt 46) 
    assertEqual "Soma lista" expectedOutput (fst output)

testSomaParSimples :: Test
testSomaParSimples = TestCase $ do
    output <- processFile (executeProg ["soma"] []) "src/Language/Analysis/Pares.lng" inputInt
    let expectedOutput = (ValorInt 30) 
    assertEqual "Soma par (x, 20)" expectedOutput (fst output)

testSomaPar :: Test
testSomaPar = TestCase $ do
    output <- processFile (executeProg ["soma"] []) "src/Language/Analysis/Pares-inputPair.lng" inputPair
    let expectedOutput = (ValorInt 30) 
    assertEqual "Soma par (x, y)" expectedOutput (fst output)

testBool :: Test
testBool = TestCase $ do
    output <- processFile (executeProg ["or"] []) "src/Language/Analysis/Bool.lng" inputBool
...
testConcatSimples = TestCase $ do
    output <- processFile (executeProg ["concat"] []) "src/Language/Analysis/Concat.lng" inputString
...
testConcatLista = TestCase $ do
    output <- processFile (executeProg ["concat"] []) "src/Language/Analysis/Concat-inputList.lng" inputListString
...
testPolymorphicPairSameType = TestCase $ do
    output <- processFile (executeProg ["invert"] []) "src/Language/Analysis/Polymorphic-pair.lng" (
...
testPolymorphicList = TestCase $ do
    output <- processFile (executeProg ["length"] []) "src/Language/Analysis/Polymorphic-list.lng" (
                    ValorList [
                      ValorStr "test",
                      ValorInt 1,
                      ValorBool True,
